    ...
    authenticate("auth-jwt") {
            ...
            route(PUSH_FAIL_PATH) {
                get {
                    if (!call.request.queryParameters[Const.userId].isNullOrBlank()) {
                        ...
                        val userOrderPushFails: List<StoredOrderPushFailed> =
                            OrderService.getUserOrderList(userId)?.orderIds?.mapNotNull { orderId ->
        ...
        route(STRIPE_WEBHOOK_PATH) {
            post {
                ...
                val httpStatusCode =
                    event?.let { OrderService.handleWebhookEvent(it) } ?: call.respond(HttpStatusCode.BadRequest)
        ...
        route(STRIPE_WEBHOOK_TEST_PATH) {
            post {
                ...
                val httpStatusCode = event?.let { OrderService.handleWebhookEvent(it, testOrder = true) }
                    ?: call.respond(HttpStatusCode.BadRequest)
                call.respond(httpStatusCode)
